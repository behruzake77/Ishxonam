...
export const AgentDetailModal: React.FC<AgentDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 select-none animate-fadeIn">
      <div className="w-full max-w-2xl bg-gray-950 border border-cyan-500/30 rounded-2xl shadow-2xl overflow-hidden font-sans">
        {/* Header */}
        <div 
          className="p-5 border-b border-cyan-500/20 relative"
          style={{ background: `linear-gradient(135deg, ${agent.avatarColor}20 0%, rgba(3, 7, 18, 0.95) 100%)` }}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div 
                className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg border border-white/20"
          ...
          <div className="grid grid-cols-4 gap-2 mt-4 pt-3 border-t border-gray-800/60 font-mono text-xs">
            <div className="p-2 rounded-lg bg-gray-900/60 border border-gray-800">
              <div className="text-[9px] text-gray-400">AI MODEL</div>
              <div className="text-cyan-300 font-bold truncate">{agent.aiModel}</div>
            </div>
            <div className="p-2 rounded-lg bg-gray-900/60 border border-gray-800">
              <div className="text-[9px] text-gray-400">CPU LOAD</div>
              <div className="text-purple-300 font-bold">{agent.cpuLoad}%</div>
            </div>
            <div className="p-2 rounded-lg bg-gray-900/60 border border-gray-800">
              <div className="text-[9px] text-gray-400">TOKENS / SEC</div>
        ...
        {/* Tab Selection */}
        <div className="flex border-b border-gray-900 bg-gray-900/40 px-4 pt-2 gap-2 text-xs font-mono">
          <button
            onClick={() => setActiveTab('tasks')}
            className={`px-4 py-2 rounded-t-xl font-bold flex items-center gap-1.5 transition-all ${
              activeTab === 'tasks' ? 'bg-gray-950 text-cyan-400 border-t border-x border-cyan-500/30' : 'text-gray-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-t-xl font-bold flex items-center gap-1.5 transition-all ${
              activeTab === 'logs' ? 'bg-gray-950 text-cyan-400 border-t border-x border-cyan-500/30' : 'text-gray-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-t-xl font-bold flex items-center gap-1.5 transition-all ${
              activeTab === 'memory' ? 'bg-gray-950 text-cyan-400 border-t border-x border-cyan-500/30' : 'text-gray-400 hover:text-white'
            }`}
          >
        ...
        {/* Tab Body */}
        <div className="p-5 max-h-[50vh] overflow-y-auto custom-scrollbar font-mono text-xs space-y-4">
          {activeTab === 'tasks' && (
            <div className="space-y-3">
              <div className="p-3 rounded-xl bg-gray-900/60 border border-gray-800 space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-white text-sm">{agent.currentTask.title}</span>
              ...
              <div className="space-y-1.5">
                <div className="text-[11px] text-gray-400 uppercase tracking-wider">Subtask Breakdown</div>
          ...
          {activeTab === 'logs' && (
            <div className="bg-gray-950 p-3 rounded-xl border border-gray-800 space-y-2 text-[11px] font-mono">
              {agent.logs.map((log) => (
                <div key={log.id} className="flex items-center justify-between text-gray-300 border-b border-gray-900 pb-1">
        ...
        <div className="p-4 border-t border-gray-900 bg-gray-950 flex items-center justify-between">
          <button
            ...
            className="px-3 py-2 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30 text-xs font-semibold flex items-center gap-1.5"
          >
            <Coffee className="w-4 h-4" />
            Send to Bio-Coffee Lounge
          </button>
          <button
            onClick={onClose}
